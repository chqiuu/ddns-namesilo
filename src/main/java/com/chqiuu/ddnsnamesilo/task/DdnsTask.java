...
public class DdnsTask {
    ...
    public void ddnsTask() {
        ...
        if (localIp.equals(Constant.Public.DOMAIN_IP_ADDRESS)) {
            ...
            dnsUpdateRecord(localIp);
        }
    }

    private void dnsUpdateRecord(String localIp) {
        // 修改域名映射IP
        try {
            String mainDomain = getMainDomain(properties.getDomains()[0]);
            Constant.Public.DOMAIN_MAP.forEach((recordId, domain) -> {
                String url = String.format("https://www.namesilo.com/api/dnsUpdateRecord?version=1&type=xml&key=%s&domain=%s&rrid=%s&rrhost=%s&rrvalue=%s&rrttl=3601", properties.getNamesiloApiKey(), mainDomain, recordId, domain, localIp);
                log.info("设置url {}", url);
