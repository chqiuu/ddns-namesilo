package com.chqiuu.ddnsnamesilo.config.interceptor;
...
public class TimeConsumingInterceptor implements AsyncHandlerInterceptor {
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, @Nullable Exception ex) throws Exception {
        ...
        if (elapsedTimeMillis > SLOW_MIN_TIME_MILLIS) {
            log.warn("SessionId: {} {} {}；耗时：{}s", request.getSession().getId(), ServletUtil.getClientIP(request, ""), getFullUrl(request), elapsedTimeMillis / 1000);
        } else {
            log.info("SessionId: {} {} {}；耗时：{}s", request.getSession().getId(), ServletUtil.getClientIP(request, ""), getFullUrl(request), elapsedTimeMillis / 1000);
        }
    }
